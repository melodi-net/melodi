                          ...
                          const u8 *message, size_t message_size)
{
	...
	if (ctx->input_idx == 128) {
		sha512_incr(ctx->input_size, 1024);
		sha512_compress(ctx);
		ctx->input_idx = 0;
		ZERO(ctx->input, 16);
	}
                             ...
                             const u8 *message, size_t message_size)
{
	...
	crypto_eddsa_trim_scalar(a, a);
	hash_reduce(r, dom, dom_size, a + 32, 32, message, message_size, NULL, 0);
...
void crypto_ed25519_sign(u8 signature [64], const u8 secret_key[64],
                         const u8 *message, size_t message_size)
{
	ed25519_dom_sign(signature, secret_key, NULL, 0, message, message_size);
}
...
{
	u8 h_ram[32];
	hash_reduce(h_ram, signature, 32, public_key, 32, msg, msg_size, NULL, 0);
